...
def execGlobalLocalUNet(model, input_np, tile_size=512, overlap=64, device='cpu'):
	...
	full_resized   = cv2.resize(input_np, (tile_size, tile_size))
	...
	with torch.no_grad():
		...
		for y in ys:
			for x in xs:
				...
				tensor  = torch.from_numpy(tile).unsqueeze(0).unsqueeze(0).float() / 127.5 - 1.0
				tensor  = tensor.to(device)
				
				pred    = model.local_branch(tensor, global_features)
